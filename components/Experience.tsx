...
interface ExperienceProps {
  ...
  onMorphReady?: (fn: () => void) => void;
}

export const Experience: React.FC<ExperienceProps> = ({ mode, handPosition, uploadedPhotos, onSelectPhoto, onMorphReady }) => {

  const controlsRef = useRef<any>(null);
  const morphControl = useRef<{ triggerMorph: () => void } | null>(null);

  useEffect(() => {
    if (onMorphReady && morphControl.current) {
      onMorphReady(morphControl.current.triggerMorph);
    }
  }, [onMorphReady]);
  ...
  return (
    <>
      ...
      {/* Background Particles - Scaled up and pushed back */}
      <group scale={[2, 2, 2]} position={[0, 0, -50]}>
        <MorphingParticles
          onRef={(ref) => {
            morphControl.current = ref;
            if (onMorphReady) onMorphReady(ref.triggerMorph);
          }}
        />
      </group>

      {/* Foreground Christmas Tree */}
      <group position={[0, -5, 0]}>
        <Foliage mode={mode} count={12000} />
        <Ornaments mode={mode} count={600} />
        <Polaroids mode={mode} uploadedPhotos={uploadedPhotos} onSelectPhoto={onSelectPhoto} />
        <TreeStar mode={mode} />
